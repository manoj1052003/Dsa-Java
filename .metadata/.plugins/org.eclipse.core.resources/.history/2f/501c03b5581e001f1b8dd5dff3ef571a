...
class Solution {
    public int solution(int[] A) {
        ...
        int result = 0;
        if (N - 1 >= 3) {
            result = max3Tiles[N - 2];
        } else if (N - 1 == 2) {
            result = max2Tiles[N - 2];
        } else {
            result = max1Tile[N - 2];
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sol.solution(new int[]{2, 3, 5, 2, 3, 4, 6, 4, 1})); // Output: 25
        System.out.println(sol.solution(new int[]{1, 5, 3, 2, 6, 6, 10, 4, 7, 2, 1})); // Output: 35
        System.out.println(sol.solution(new int[]{1, 2, 3, 3, 2})); // Output: 10
        System.out.println(sol.solution(new int[]{5, 10, 3})); // Output: 15
    }
}
