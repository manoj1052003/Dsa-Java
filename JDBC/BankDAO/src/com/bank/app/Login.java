		...
		if(acc == 1100110011) {
			...
			do {
				...
				case 3:
					...
					boolean res = cdao.deleteCustomer(acc_no);
					Transaction t1 = new Transaction();
					t1.setTransaction_id(tid.getTranID());
					t1.setUser_acc(acc_no);
					t1.setRec_acc(c.getAcc_no());
					t1.setAmount(c1.getBalance());
					t1.setTransaction_type("CREDITED");
					t1.setBalance(c.getBalance());
					boolean res1 = tdao.insertTransaction(t1);
					if(res && res1) {
				
						System.out.println("delete agide guru");
					}
					else {
						System.out.println("no way chance  illa eno miss aagide ");
					}
			...
			do {
				...
				case 4:
					//p.print(c);
					ArrayList<Transaction> arr1 =tdao.getTransaction(c.getAcc_no());
					for(Transaction obj : arr1) {
						System.out.println("==============================");
						System.out.println("user" + obj.getUser_acc());
						System.out.println("recver"+obj.getRec_acc());
